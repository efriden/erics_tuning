...
def _save_layout(new_layout: dict) -> None:
    ...
    if APP_CONFIG_PATH.is_file():
        try:
            ...
            logger.exception("AnalyzerLayoutScreen: failed to read config.json for saving; overwriting")
            cfg = {}
    ...
    tuning_config_rel = cfg.get("analyzer", {}).get("config", "analyzer/tuning_config.json")
    ...
    try:
        ...
        tuning_config_path.write_text(json.dumps(tuning_cfg, indent=2), encoding="utf-8")
        logger.info("Auto-set beat_detection_enabled=%s based on selected graphs", beat_detection_needed)
    except Exception:
